    ...
    public static void searchByRegexTitle( Inventory inventory, String regexType ) {
        ...
        try {
            ...
            for ( Integer key : inventory.getInventory().keySet() ) {
                ...
                switch( regexType ) {
                    case "find"  ->  { if ( Regex.isFound( inventoryEntry.getItem().getName(), pattern) ) println( inventoryEntry ); }
                    case "match" ->  { if ( Regex.isMatch( inventoryEntry.getItem().getName(), pattern) ) println( inventoryEntry ); }
                }
            }
        }
